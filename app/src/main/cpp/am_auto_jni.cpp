		...
		jobject buffer)
{
	...
	if (is_active && !left_boxes.empty())
	{
		...
		env->CallVoidMethod(obj, click_id, key_xy.x + key_xy.width / 2,
	...
	} else if (pressing)
	{
		...
		threshold(grey, thresholded, 180, 255, THRESH_BINARY);
		...
		for (auto &ct: contours)
		{
			...
			if (r.height > 20)
				bboxes.push_back(boundingRect(ct));
		}
		std::sort(bboxes.begin(), bboxes.end(), [](const Rect &r1, const Rect &r2) -> bool
		{
			return abs(1 - float(r1.width) / r1.height) < abs(1 - float(r2.width) / r2.height);
		});
		if (!bboxes.empty())
		{
			Rect r = bboxes[0];
			...
			{
				env->CallVoidMethod(obj, click_id,
									k4_config.space_key.x + k4_config.space_key.width / 2,
